#!/usr/bin/env bash
# =============================================================================
# Cranfield University runway microclimate CFD pipeline
# City4CFD → snappyHexMesh → simpleFoam (kEpsilon, steady-state)
#
# The CFD frame is rotated so the x axis runs along runway 03/21
# (true bearing ~034°). Wind blows from 214° (prevailing SW), i.e. straight
# down the runway — the reason the runway was built on that heading.
#
# Usage:
#   ./pipeline.sh [STAGE]
#
...
#   osm        – download OpenStreetMap data (buildings, aeroways, landuse)
#   geometry   – run City4CFD to generate OBJ surfaces
#   stl        – rotate runway → +x and merge OBJ buildings → buildings.stl
...
# =============================================================================
stage_osm() {
    log "Downloading OSM data (buildings, runway, vegetation, water)"
    require_venv
    cd "$SCRIPT_DIR"
    mkdir -p data
    "$VENV/bin/python3" fetch_osm_cranfield.py
}
...
# =============================================================================
stage_stl() {
    log "Rotating to runway frame and converting OBJ → STL"
    require_venv
    cd "$SCRIPT_DIR"
    mkdir -p "$OF_CASE/constant/triSurface"
    "$VENV/bin/python3" rotate_and_merge_stl.py \
        --results results \
        --runway data/cranfield_runway.geojson \
